...
| `@deepseek-ai/dsh-tool-web` | `web_fetch`, `web_search` | `ctx.tools`, `ctx.web`, `ctx.systemPrompt` | `tool/call`, `tool/result` | - | web_search 使用 SearXNG，web_fetch 使用 ctx.web 后面的 HTTP 提供方；更换后端不会改变模型可见的工具 schema。 |
...
通过 SearXNG 搜索当前网络信息。必须在 queries 数组中提供 1–4 个查询；返回来源 URL 和摘要。
...
web_search 使用 SearXNG，web_fetch 使用 ctx.web 后面的 HTTP 提供方；更换后端不会改变模型可见的工具 schema。
