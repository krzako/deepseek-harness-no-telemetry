...
- `presentResult(args, { content, isError, meta? })` returns the completed card:
  ...
  - `web` supplies a completed web retrieval, discriminated by `kind: 'fetch'` (the fetch summary), derived from `result.meta`; it carries no body copy, so a UI without the `web` capability falls back to the raw result content. (tool-web `web_fetch`.)
