...
打开设置中的「插件」分区并选择**插件配置**标签页，即可编辑本部署所组装的宿主平面插件。卡片依次为 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`）、管理员网页搜索上限（`web-search`）、SearXNG 提供方选项（`web-search-searxng`）以及 subagent 模型选择（`subagent-model-selection`）。
...
网页搜索卡片展示合并结果上限、查询上限、Host 全局并发请求上限与超时；每个字段只接受正整数。SearXNG 卡片展示 Host 适配器接受的全部选项：实例 URL、语言、逗号分隔的类别和安全搜索级别。URL 与安全搜索草稿会在保存前校验；提供方每次请求都会读取实时 settings 来源，因此通过校验的修改会从下一次搜索开始生效。**测试连接**会通过已保存的 Host 配置执行一次小型搜索，并在卡片中报告成功或失败。存在待保存修改时必须先保存，因此测试结果始终对应界面显示为已持久化的值。
